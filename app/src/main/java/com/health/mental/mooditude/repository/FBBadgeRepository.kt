 ...
 */
class FBBadgeRepository(private val mAppDb: AppDatabase,
            ...
            override fun onDataChange(p0: DataSnapshot) {
                if (p0.exists()) {
                    ...
                    for (key in map.keys) {
                        ...
                        list.add(Reward.fromApiData(reward))

                        saveToLocal(list)
                        //Now save this data to db
                        //val journalPromptRecord = JournalPrompt.readFromApi(reward)
                        //printLog("meditationCatRecord : " + journalPromptRecord.toString())
